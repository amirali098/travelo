...
def single_blog(request,id):
    ...
    if post.login_requier==False:
        ...
        next_post = Post.objects.filter(id__gt=post.id, status=1, published_date__lte=timezone.now()).order_by(
            'id').first()
        previous_post = Post.objects.filter(id__lt=post.id, status=1, published_date__lte=timezone.now()).order_by(
            'id').first()
        post.counted_views += 1
        post.save()
        comments = Comment.objects.filter(post=post.id, approved=True).order_by("-created_date")
